...
PGMaps exposes this under its separate `postal` source. `postalRegion` is BC region V; `postalPrefix2` is the 10 two-character groups V0–V9; `fsa` is the three-character child level. `build-postal-regions.mjs`, also invoked by the main sync, dissolves the existing FSA display layer by one- and two-character prefixes to generate both parent levels. No independent download or second simplification is involved. The two-character groups are 2.49 MB gzip. The BC parent is 2.25 MB gzip and its manifest records the exact child snapshot checksum. Validation asserts each group exactly equals its child FSA union, all groups together equal the BC parent, and groups do not overlap. Two-character groups are PGMaps-defined and may be disconnected. Four-, five-, and six-character postal-code polygons are not available; observation-prefix grouping cannot derive these polygons.
