...
function parseToleranceMetres() {
  ...
  const raw = arg ? arg.split('=')[1] : process.env.DRAINAGE_BASIN_SIMPLIFY_TOLERANCE_METRES
  if (!raw) return DEFAULT_TOLERANCE_METRES

  const parsed = Number(raw)
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`Invalid drainage-basin simplification tolerance in metres: ${raw}`)
  }
  return parsed
}
